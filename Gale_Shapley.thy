...
fun progress_prop_idxs::"nat list \<Rightarrow> man \<Rightarrow> nat list" where
"progress_prop_idxs next_prop_idxs m = list_update next_prop_idxs m (next_prop_idxs!m+1)"
...
"Gale_Shapley' N MPrefs WPrefs 
 ...
 = (case findFreeMan engagements of 
  ...
  Some m \<Rightarrow> (let w = (MPrefs!m)!(next_prop_idxs!m) in (
    case findFiance engagements w of
      None \<Rightarrow> Gale_Shapley' N MPrefs WPrefs 
        (list_update engagements m (Some w)) (progress_prop_idxs next_prop_idxs m)
    | Some m' \<Rightarrow> (
      if prefers w WPrefs m m' then Gale_Shapley' N MPrefs WPrefs
        (list_update (list_update engagements m (Some w)) m' None) (progress_prop_idxs next_prop_idxs m)
      else Gale_Shapley' N MPrefs WPrefs
        engagements (progress_prop_idxs next_prop_idxs m)
    )
  ))
)"
